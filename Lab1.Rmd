...
df_with_categories <- tidy_text %>% count(label, word, sort=TRUE)
spam_df <- df_with_categories %>% filter(label=="discrim")
non_spam_df <- df_with_categories %>% filter(label=="neutral")
```
...
is_discr = nrow(train %>% filter(label=="neutral"))/nrow(train)
is_netral = 1 - is_discr
```
       ...
       methods = list(
                    fit = function()
                    {
                      for (row in 1:nrow(discrim_test)){
                        val = predict(discrim_test[row, -4])
                        if (val == 1){
                          discrim_test[row, "result"] = 0
                        }
                      }
                      for (row in 1:nrow(neutral_test)){
                        val = predict(neutral_test[row, -4])
                        if (val == 0){
